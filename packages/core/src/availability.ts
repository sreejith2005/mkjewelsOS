export type AvailabilityRange = Readonly<{ startDate: string; endDate: string }>;

export function normalizeAvailabilityRange(startDate: string, endDate: string): AvailabilityRange {
  ...
  if ((end.getTime() - start.getTime()) / 86_400_000 > 366) {
    throw new Error("Availability range cannot exceed 367 days");
  }
