...
export async function hashTaskImportPayload(payload: TaskBulkImportPayload): Promise<string> { const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(taskImportPayloadHashSource(payload))); return [...new Uint8Array(digest)].map((item) => item.toString(16).padStart(2, "0")).join(""); }
