...
/**
 * Permission management, shared by both clients. This is the web
 * `features/permissions/api.ts` contract on the shared client; every write is
 * an audited, Super Admin-only RPC and the database stays the authority.
 */
...
export async function fetchPermissionAdminContext(): Promise<PermissionAdminContext> {
  const { data, error } = await db().rpc("get_permission_admin_context");
...
export async function fetchUserAccessBreakdown(profileId: string): Promise<UserAccessBreakdown> {
  const { data, error } = await db().rpc("get_user_access_breakdown", { p_profile_id: profileId });
  if (error) throw error;
...
export async function saveRolePermissions(role: UserRole, changes: RolePermissionChanges): Promise<void> {
  const { error } = await db().rpc("save_role_permissions_with_audit", { p_role: role, p_permissions: changes });
  if (error) throw error;
}

export async function saveDesignationPermissions(designationId: string, changes: OverrideChanges): Promise<void> {
  const { error } = await db().rpc("save_designation_permissions_with_audit", { p_designation_id: designationId, p_overrides: changes });
  if (error) throw error;
}
...
export async function saveUserAccess(profileId: string, authority: DashboardAuthority | null, changes: OverrideChanges): Promise<UserAccessBreakdown> {
  const { data, error } = await db().rpc("save_user_access_with_audit", {
