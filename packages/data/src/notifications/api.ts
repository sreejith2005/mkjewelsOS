...
function createInboxRealtimeSubscription(profileId: string): InboxRealtimeSubscription {
  ...
  const channel = db().channel(`notifications:${profileId}`).on(
    "postgres_changes",
...
export async function loadInbox(profileId: string, limit = 200): Promise<InboxNotification[]> {
  const { data, error } = await db().from("notifications").select("id,event_type,title,message,link_url,is_read,read_at,priority,created_at").eq("user_profile_id", profileId).order("created_at", { ascending: false }).limit(limit);
...
export async function markNotification(notificationId: string, isRead: boolean): Promise<void> {
  const { error } = await db().rpc("mark_notification_read", { p_notification_id: notificationId, p_is_read: isRead });
  if (error) throw error;
}
  ...
  return () => {
    ...
    subscription.removalTimer = setTimeout(() => {
      if (subscription && subscription.listeners.size === 0) {
        ...
        void db().removeChannel(subscription.channel);
      }
    }, 0);
  };
}
...
export async function loadTemplates(): Promise<NotificationTemplateRow[]> {
  const { data, error } = await db().from("notification_templates").select("id,name,event_type,channel,title_template,body_template,link_url,is_active,lifecycle").order("created_at", { ascending: false });
  if (error) throw error;
...
export async function saveTemplate(input: Omit<NotificationTemplateRow, "id" | "lifecycle"> & { id?: string }): Promise<string> {
  const { data, error } = await db().rpc("save_notification_template", {
...
export async function archiveTemplate(id: string): Promise<void> {
  const { error } = await db().rpc("archive_notification_template", { p_template_id: id });
  if (error) throw error;
}

export async function loadRules(): Promise<NotificationRuleRow[]> {
  const { data, error } = await db().from("notification_rules").select("id,name,event_type,conditions,recipient_rules,channel_templates,delay_minutes,cooldown_minutes,max_attempts,backoff_minutes,priority,is_active,lifecycle").order("created_at", { ascending: false });
  if (error) throw error;
  return (data ?? []) as NotificationRuleRow[];
}

export async function saveRule(input: RuleDraft): Promise<string> {
  const { data, error } = await db().rpc("save_notification_rule", {
...
export async function setRuleEnabled(id: string, enabled: boolean): Promise<void> {
  const { error } = await db().rpc("set_notification_rule_enabled", { p_rule_id: id, p_enabled: enabled });
  if (error) throw error;
}

export async function archiveRule(id: string): Promise<void> {
  const { error } = await db().rpc("archive_notification_rule", { p_rule_id: id });
  if (error) throw error;
}

export async function loadProviders(): Promise<ProviderAvailability[]> {
  const { data, error } = await db().rpc("get_notification_provider_availability");
  if (error) throw error;
...
export async function loadDeliveryLogs(filters: { state?: string; channel?: string; eventType?: string; search?: string }): Promise<DeliveryLog[]> {
  const { data, error } = await db().rpc("list_notification_delivery_logs", {
...
export async function retryDelivery(id: string): Promise<void> {
  const { error } = await db().rpc("retry_notification_delivery", { p_delivery_id: id });
  if (error) throw error;
}

export async function loadActiveRecipientProfiles(): Promise<Array<{ id: string; employee_name: string; user_role: string }>> {
  const { data, error } = await db().from("user_profiles").select("id,employee_name,user_role").in("account_status", ["active", "invited"]).eq("working_status", "active").order("employee_name");
  if (error) throw error;
  return data ?? [];
}
