...
export async function loadFmsBuilderData(): Promise<FmsData> {
  ...
  const results = await Promise.all([
    db().from("fms_flows").select("*").order("name").order("version", { ascending: false }).limit(300),
    db().from("fms_stages").select("*").order("sort_order").limit(1000),
    db().from("fms_stage_assignees").select("*").order("sort_order").limit(1000),
    db().from("fms_branch_rules").select("*").order("sort_order").limit(1000),
    db().from("form_templates").select("id,name,version,family_id,lifecycle").eq("lifecycle", "published").eq("is_active", true).order("name").limit(300),
    db().from("dropdown_masters").select("label,value").eq("master_type", "buy_status").eq("is_active", true).order("sort_order").limit(300),
    db().from("user_profiles").select("id,employee_name,employee_code,account_status,user_role,branch_id,department_id,working_status,is_login_enabled").order("employee_name").limit(500),
    db().from("branches").select("id,name").eq("is_active", true).order("name").limit(100),
    db().from("departments").select("id,branch_id,name").eq("is_active", true).order("name").limit(300),
    db().from("user_availability").select("user_profile_id,status").eq("date", kolkataDate).limit(1000),
    db().from("fms_context_assignee_defaults").select("module_context,user_profile_id").limit(100),
  ]);
...
async function withPinnedFormVersions(published: readonly FormRef[], stages: readonly FmsStageRow[]): Promise<FormRef[]> {
  ...
  const { data, error } = await db().from("form_templates").select("id,name,version,family_id,lifecycle").in("id", pinned).limit(300);
...
export async function saveFmsDraft(flowId: string | null, definition: FmsFlowDefinition): Promise<string> {
  ...
  const { data, error } = await db().rpc("save_fms_flow_draft_with_audit", { p_flow_id: flowId as string, p_metadata: metadata as Json, p_stages: definition.stages as unknown as Json }); fail("Save FMS draft", error); if (!data) throw new Error("Save FMS draft returned no ID"); const { error: contextError } = await db().rpc("set_fms_flow_context_with_audit", { p_flow_id: data, ...(definition.moduleContext ? { p_module_context: definition.moduleContext } : {}) }); fail("Save FMS workflow context", contextError); return data;
}
export const saveFmsContextAssigneeDefault = async (moduleContext: string, userProfileId: string) => { const { error } = await db().rpc("save_fms_context_assignee_default_with_audit", { p_module_context: moduleContext, p_user_profile_id: userProfileId }); fail("Save FMS default assignee", error); };
export const reviseFmsFlow = async (id: string) => { const { data, error } = await db().rpc("create_fms_revision_with_audit", { p_flow_id: id }); fail("Create FMS revision", error); return data; };
export const publishFmsFlow = async (id: string) => { const { error } = await db().rpc("publish_fms_flow_with_audit", { p_flow_id: id }); fail("Publish FMS flow", error); };
export const archiveFmsFlow = async (id: string, reason: string) => { const { error } = await db().rpc("archive_fms_flow_with_audit", { p_flow_id: id, p_reason: reason }); fail("Archive FMS flow", error); };
export const setFmsFlowActive = async (id: string, active: boolean, reason?: string) => { const { error } = await db().rpc("set_fms_flow_active_with_audit" as never, { p_flow_id: id, p_active: active, ...(reason ? { p_reason: reason } : {}) } as never); fail(active ? "Resume FMS flow" : "Pause FMS flow", error); };
export const restoreFmsFlow = async (id: string, reason?: string) => { const { error } = await db().rpc("restore_fms_flow_with_audit" as never, { p_flow_id: id, ...(reason ? { p_reason: reason } : {}) } as never); fail("Restore FMS flow", error); };
export const deleteFmsFlow = async (id: string) => { const { error } = await db().rpc("delete_fms_flow_with_audit" as any, { p_flow_id: id }); fail("Delete FMS flow", error); };
...
export async function loadFmsRuntime(): Promise<{ instances: FmsInstance[]; stages: FmsInstanceStage[]; definitions: FmsStageRow[]; flows: FmsFlowRow[]; checklist: FmsChecklistItem[]; evidence: FmsEvidence[]; logs: FmsLog[]; users: FmsData["users"] }> {
  const [instances, stages, definitions, flows, checklist, evidence, logs, users] = await Promise.all([
    db().from("fms_instances").select("*").order("started_at", { ascending: false }).limit(200), db().from("fms_instance_stages").select("*").order("created_at").limit(1500),
    db().from("fms_stages").select("*").order("sort_order").limit(1000), db().from("fms_flows").select("*").limit(300), db().from("fms_instance_checklist_items").select("*").order("sort_order").limit(2000),
    db().from("fms_evidence").select("*").is("removed_at", null).order("created_at").limit(1000), db().from("fms_stage_logs").select("*").order("created_at").limit(3000),
    db().from("user_profiles").select("id,employee_name,user_role,branch_id,department_id,working_status,is_login_enabled").order("employee_name").limit(500),
...
export async function startFmsInstance(input: { flowId: string; title: string; priority: "high" | "medium" | "low"; context: Json; branchId: string; departmentId: string; firstAssigneeId: string | null }): Promise<FmsStartResult> { const { data, error } = await db().rpc("start_fms_instance_with_audit", { p_flow_id: input.flowId, p_title: input.title, p_priority: input.priority, p_context: input.context, p_branch_id: input.branchId, p_department_id: input.departmentId, ...(input.firstAssigneeId ? { p_first_assignee_id: input.firstAssigneeId } : {}) }); fail("Start FMS instance", error); const result = data?.[0]; if (!result) throw new Error("Start FMS instance returned no reference"); return result; }
export const claimFmsStage = async (id: string) => { const { error } = await db().rpc("claim_fms_stage_with_audit", { p_instance_stage_id: id }); fail("Claim FMS stage", error); };
export const completeFmsStage = async (id: string, outcome: string, remark: string, checklist: Record<string, boolean>, nextAssigneeId: string | null) => { const { error } = await db().rpc("complete_fms_stage_with_audit", { p_instance_stage_id: id, p_checklist: checklist, ...(outcome ? { p_outcome: outcome } : {}), ...(remark ? { p_remark: remark } : {}), ...(nextAssigneeId ? { p_next_assignee_id: nextAssigneeId } : {}) }); fail("Complete FMS stage", error); };
...
export const reviewFmsStage = async (id: string, decision: "approved" | "rejected" | "revision_requested", remark: string, nextAssigneeId: string | null) => { const { error } = await db().rpc("review_fms_stage_with_audit", { p_instance_stage_id: id, p_decision: decision, ...(remark ? { p_remark: remark } : {}), ...(nextAssigneeId ? { p_next_assignee_id: nextAssigneeId } : {}) }); fail("Review FMS stage", error); };
export const reassignFmsStage = async (id: string, from: string, to: string, reason: string) => { const { error } = await db().rpc("reassign_fms_stage_with_audit", { p_instance_stage_id: id, p_from_user_id: from, p_to_user_id: to, p_reason: reason }); fail("Reassign FMS stage", error); };
export const moveFmsStageBackward = async (id: string, target: string, reason: string, assignee: string | null) => { const { error } = await db().rpc("move_fms_stage_backward_with_audit", { p_instance_stage_id: id, p_target_stage_id: target, p_reason: reason, ...(assignee ? { p_assignee_id: assignee } : {}) }); fail("Move FMS stage backward", error); };
export const requestFmsRevision = async (id: string, target: string, reason: string, assignee: string | null) => { const { error } = await db().rpc("request_fms_revision_with_audit", { p_instance_stage_id: id, p_target_stage_id: target, p_reason: reason, ...(assignee ? { p_assignee_id: assignee } : {}) }); fail("Request FMS revision", error); };
export const escalateFmsStage = async (id: string, reason: string) => { const { error } = await db().rpc("escalate_fms_stage_with_audit", { p_instance_stage_id: id, p_reason: reason }); fail("Escalate FMS stage", error); };
export const setFmsInstanceStatus = async (id: string, action: "hold" | "resume" | "cancel", reason: string) => {
  ...
  const { error } = await db().rpc(rpc, { p_instance_id: id, p_reason: reason });
  fail(`${action} FMS instance`, error);
};
export const updateFmsChecklistItem = async (id: string, completed: boolean) => { const { error } = await db().rpc("update_fms_checklist_item_with_audit", { p_item_id: id, p_completed: completed }); fail("Update FMS checklist", error); };
export async function uploadFmsEvidence(stageId: string, tenantId: string, file: UploadSource) { const extension = file.name.toLowerCase().match(/\.[^.]+$/)?.[0]; const allowed = new Set([".jpg", ".jpeg", ".png", ".webp", ".pdf"]); if (!extension || !allowed.has(extension) || file.size < 1 || file.size > 10 * 1024 * 1024 || !["image/jpeg", "image/png", "image/webp", "application/pdf"].includes(file.type)) throw new Error("Evidence must be a JPG, PNG, WebP, or PDF up to 10 MB"); const path = `${tenantId}/${stageId}/${newRequestKey()}${extension}`; const { error: uploadError } = await db().storage.from("fms-evidence").upload(path, uploadBody(file), { contentType: file.type, upsert: false }); fail("Upload FMS evidence", uploadError); const { error } = await db().rpc("register_fms_evidence_with_audit", { p_instance_stage_id: stageId, p_storage_path: path, p_original_filename: file.name.slice(0, 240), p_mime_type: file.type, p_size_bytes: file.size }); fail("Register FMS evidence", error); }
export async function signedFmsEvidenceUrl(path: string) { const { data, error } = await db().storage.from("fms-evidence").createSignedUrl(path, 60); fail("Open FMS evidence", error); if (!data) throw new Error("Signed URL was not created"); return data.signedUrl; }
