...
export { EMPTY_RECURRING_STATS, parseRecurringWorkspace, recurringInstanceDisplayStatus, recurringInstanceNeedsWork, type RecurringInstance, type RecurringStats, type RecurringTemplate, type RecurringWorkspace } from "./model";

/** Mirrors the filter set `get_recurring_todo_workspace` accepts. */
export type RecurringWorkspaceFilter = {
  date_from: string;
  date_to: string;
  search?: string;
  status?: string;
...
export async function loadRecurringWorkspace(filter: RecurringWorkspaceFilter): Promise<RecurringWorkspace> {
  const { data, error } = await db().rpc("get_recurring_todo_workspace", { p_filter: filter });
...
export async function saveRecurringTemplate(id: string | null, payload: Json): Promise<string> {
  const { data, error } = await db().rpc("save_recurring_todo_template_with_audit", { p_template_id: id as string, p_payload: payload });
...
export async function materializeRecurringTemplate(
  ...
  const { data, error } = await db().functions.invoke("materialize-recurring-schedule", {
  ...
  const { data: taskId, error: fallbackError } = await db().rpc("run_recurring_todo_template_now_with_audit", {
...
export async function deleteRecurringTemplate(id: string): Promise<string> {
  const { data, error } = await db().rpc("delete_recurring_todo_template_with_audit", { p_template_id: id });
...
export async function setRecurringTemplateActive(id: string, active: boolean): Promise<void> {
  const { error } = await db().rpc("set_recurring_todo_template_active_with_audit", { p_template_id: id, p_active: active });
...
export async function runRecurringTemplateNow(id: string, targetDate: string): Promise<void> {
  const { error } = await db().rpc("run_recurring_todo_template_now_with_audit", { p_template_id: id, p_target_date: targetDate });
...
export async function verifyRecurringTask(id: string, decision: "verified" | "rejected", note: string): Promise<void> {
  const { error } = await db().rpc("verify_recurring_task_with_audit", { p_task_id: id, p_decision: decision, p_note: note });
...
export async function sendRecurringFollowup(id: string, message: string): Promise<void> {
  const { error } = await db().rpc("send_recurring_followup_with_audit", { p_task_id: id, p_message: message });
