...
export async function fetchTaskEvidenceWorkspace(filter: Record<string, string | number | undefined>): Promise<EvidenceWorkspace> {
  const { data, error } = await (db().rpc as unknown as EvidenceRpc)("get_task_evidence_workspace", { p_filter: filter });
...
export async function signedTaskEvidenceUrl(attachmentId: string): Promise<string> {
  const { data, error } = await (db().rpc as unknown as PathRpc)("get_task_attachment_path", { p_attachment_id: attachmentId });
  if (error) throw new Error(error.message);
  const signed = await db().storage.from("task-attachments").createSignedUrl(data as string, 60);
