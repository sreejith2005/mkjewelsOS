...
), 376, 'exactly 376 postgres-owned public application functions exist in the current migration set');
select is((
  select count(*)::integer
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
...
), 314, 'exactly 314 public application functions are SECURITY DEFINER in the current migration set');
select is((
  select count(*)::integer
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
...
), 188, 'authenticated can execute exactly 188 reviewed public application functions');
select is((
  select count(*)::integer
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
...
), 376, 'postgres retains owner execution on every public application function');
...
select is((
  with expected(identity) as (
    select unnest(array[
      ...
      'has_permission(text)', 'module_accessible(text,boolean)',
      'assert_module_enabled(text)', 'assert_module_access(text)',
      'get_my_access_context()', 'get_permission_admin_context()',
      'get_user_access_breakdown(uuid)', 'save_role_permissions_with_audit(user_role,jsonb)',
