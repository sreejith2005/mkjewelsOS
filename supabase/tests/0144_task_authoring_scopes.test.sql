...
select ok(position('participant.branch_id = v_actor.branch_id' in pg_get_functiondef('public.create_delegation_task_with_audit(jsonb,uuid[],uuid[],jsonb)'::regprocedure)) = 0,'normal task authors can assign across branches within their department');
