...
select lives_ok($$select publish_fms_flow_with_audit((select id from fms_flows where name='Full builder workflow'))$$,'the fully assigned two-node workflow publishes');
...
select is((select fs.stage_key from fms_instance_stages fis join fms_instances fi on fi.id=fis.fms_instance_id join fms_stages fs on fs.id=fis.fms_stage_id where fi.title='Full builder run' and fis.status='in_progress'),'finish_task','the next assigned task activates');
select lives_ok($$select complete_fms_stage_with_audit((select fis.id from fms_instance_stages fis join fms_instances fi on fi.id=fis.fms_instance_id join fms_stages fs on fs.id=fis.fms_stage_id where fi.title='Full builder run' and fs.stage_key='finish_task'),null,null,'{}',null)$$,'the final assigned task completes');
