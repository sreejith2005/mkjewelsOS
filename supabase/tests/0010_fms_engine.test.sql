...
select throws_ok($$select start_fms_instance_with_audit((select id from fms_flows where name='Invited named assignee'),'Invited assignee run','medium','{}')$$,'23514',null,'an invited, disabled profile requires active coverage before its stage can start');
select is((select count(*)::int from fms_instances where title='Invited assignee run'),0,'a failed coverage check does not create a partial instance');
reset role;
...
select lives_ok($$select start_fms_instance_with_audit((select id from fms_flows where name='Invited named assignee'),'Invited assignee run','medium','{}'); select submit_form_with_audit('7a100000-0000-0000-0000-000000000001','{}','fms_stage',(select s.id from fms_instance_stages s join fms_instances i on i.id=s.fms_instance_id where i.title='Invited assignee run')); select complete_fms_stage_with_audit((select s.id from fms_instance_stages s join fms_instances i on i.id=s.fms_instance_id where i.title='Invited assignee run'),null,null,'{}',null)$$,'the activated profile can start and complete its assigned stage');
...
reset role;
update user_profiles set user_role='staff' where id='4a100000-0000-0000-0000-000000000004';
update user_profiles set buddy_id='4a100000-0000-0000-0000-000000000004' where id='4a100000-0000-0000-0000-000000000003';
insert into user_availability(tenant_id,user_profile_id,date,status,logged_by) values ('1a100000-0000-0000-0000-000000000001','4a100000-0000-0000-0000-000000000003','2099-12-31','absent','4a100000-0000-0000-0000-000000000001');
set local role authenticated; select set_config('request.jwt.claim.role','authenticated',true); select set_config('request.jwt.claim.sub','aa100000-0000-0000-0000-000000000003',true);
select lives_ok($$select start_fms_instance_with_audit((select id from fms_flows where name='FMS Runtime'),'Fallback run','medium','{}')$$,'an absent primary automatically falls back to the configured profile buddy');
select is((select assigned_to from fms_instance_stages s join fms_instances i on i.id=s.fms_instance_id where i.title='Fallback run'),array['4a100000-0000-0000-0000-000000000004'::uuid],'runtime assignment uses the configured same-scope profile buddy');
reset role;
-- Build, publish, start, and complete a real Yes/No workflow through both paths.
update user_profiles set buddy_id=null where id='4a100000-0000-0000-0000-000000000003';
update user_profiles set user_role='doer' where id='4a100000-0000-0000-0000-000000000004';
update user_availability set status='present' where user_profile_id='4a100000-0000-0000-0000-000000000003' and date in ((now() at time zone 'Asia/Kolkata')::date,'2099-12-31'::date);
set local role authenticated; select set_config('request.jwt.claim.role','authenticated',true); select set_config('request.jwt.claim.sub','aa100000-0000-0000-0000-000000000001',true);
