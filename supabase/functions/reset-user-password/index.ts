...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (!UUID_PATTERN.test(profileId)) return respond(400, { error: "profile_id must be a valid UUID" });
    if (password.length < MIN_PASSWORD_LENGTH || password.length > MAX_PASSWORD_LENGTH) return respond(400, { error: `Password must be ${MIN_PASSWORD_LENGTH}-${MAX_PASSWORD_LENGTH} characters.` });
  } catch { return respond(400, { error: "Request body must be valid JSON" }); }
  ...
  const { error: resetError } = await admin.auth.admin.updateUserById(targetAuthUserId, { password, email_confirm: true });
