...
import { normalizeLoginIdentifier } from "./identifier.ts";
...
Deno.serve(async (request) => {
  ...
  let identifier = null; let password = "";
  try { const body = await request.json(); identifier = normalizeLoginIdentifier(body?.username); password = typeof body?.password === "string" ? body.password : ""; } catch { return invalid(); }
  if (!identifier || password.length < 6 || password.length > 128) return invalid();
  const forwarded = request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() || request.headers.get("cf-connecting-ip") || "unknown";
  const key = await digest(`${rateSecret}:${forwarded}:${identifier.value}`);
  ...
  const profileQuery = admin.from("user_profiles").select("email,account_status,is_login_enabled,working_status");
  const { data: profile, error: profileError } = identifier.kind === "username"
    ? await profileQuery.eq("username", identifier.value).maybeSingle()
    : await profileQuery.eq("email", identifier.value).maybeSingle();
