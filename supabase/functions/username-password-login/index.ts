...
const invalid = () => new Response(JSON.stringify({ error: "Invalid username or password" }), { status: 401, headers });
...
Deno.serve(async (request) => {
  ...
  if (!url || !anonKey || !serviceRoleKey || !rateSecret) return respond(500, { error: "Login is temporarily unavailable" });
  ...
  if (rateError) return respond(500, { error: "Login is temporarily unavailable" });
  if (allowed !== true) return respond(429, { error: "Too many login attempts. Try again later." });
  ...
  if (profileError || !profile || profile.account_status !== "active" || !profile.is_login_enabled || profile.working_status === "resigned") return invalid();
  ...
  if (typeof session?.access_token !== "string" || typeof session?.refresh_token !== "string") return invalid();
