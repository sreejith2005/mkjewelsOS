-- User provisioning ergonomics. Email and employee name identify a new
-- account; employee code is allocated by Postgres and contact/buddy details
-- may be completed later by authorised staff.
set search_path = public, extensions;

alter table user_profiles alter column personal_mobile drop not null;

create sequence if not exists employee_code_sequence;
select setval(
  'employee_code_sequence',
  greatest(1, coalesce((select max(nullif(regexp_replace(employee_code, E'\\D', '', 'g'), '')::bigint) from user_profiles), 0) + 1),
  false
);
...
returns text language sql volatile security definer set search_path = public as $$
  select 'MK' || lpad(nextval('employee_code_sequence')::text, 4, '0');
$$;
...
begin
  ...
  if nullif(btrim(coalesce(p_personal_mobile,'')),'') is not null and btrim(p_personal_mobile) !~ '^\\+?[0-9][0-9 ()-]{7,19}$' then raise exception 'Personal mobile format is invalid' using errcode='22023'; end if;
...
revoke all on function next_employee_code() from public;
