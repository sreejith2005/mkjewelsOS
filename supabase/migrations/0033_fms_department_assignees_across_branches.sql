-- FMS assignment follows the department selected on each stage, represented
-- by its named primary/fallback. A workflow may cross departments/branches.
...
begin
 ...
 select coalesce(array_agg(distinct u.id),'{}') into v_ids from unnest(array_remove(v_ids,null)) candidate(id) join user_profiles u on u.id=candidate.id where u.tenant_id=v_instance.tenant_id and u.working_status not in ('inactive','resigned') and u.is_login_enabled;
