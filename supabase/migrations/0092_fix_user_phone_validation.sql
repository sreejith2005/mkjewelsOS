-- PostgreSQL E-strings turn the old `\+` pattern into a leading quantifier.
-- Keep the plus literal inside a character class instead.
...
begin
  ...
  if nullif(btrim(coalesce(p_personal_mobile,'')),'') is not null and btrim(p_personal_mobile) !~ '^[+]?[0-9][0-9 ()-]{7,19}$' then raise exception 'Personal phone format is invalid' using errcode='22023'; end if;
  if nullif(btrim(coalesce(p_official_mobile,'')),'') is not null and btrim(p_official_mobile) !~ '^[+]?[0-9][0-9 ()-]{7,19}$' then raise exception 'Official phone format is invalid' using errcode='22023'; end if;
