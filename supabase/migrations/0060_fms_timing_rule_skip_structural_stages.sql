-- 0060_fms_timing_rule_skip_structural_stages.sql
-- Fix: The timing rule validation in assert_fms_flow_publishable was incorrectly
-- requiring ALL stages to have a valid timing rule, including structural stages
-- (notification, branch, parallel_start, parallel_join) which don't have human
-- deadlines. This migration patches the check to only apply to human/task stages.

set search_path = public, extensions;

-- Patch assert_fms_flow_publishable: only validate timing rules for stages that
-- can actually have a meaningful SLA (i.e. not structural routing stages).
...
begin
  ...
  -- Only require a valid timing rule for human-action stages (form, task, approval, notification).
  -- Structural routing stages (branch, parallel_start, parallel_join) don't need deadlines.
  if exists(
    select 1 from fms_stages s
    where s.fms_flow_id=p_flow_id
      and s.step_type not in ('branch','parallel_start','parallel_join')
      and not is_valid_fms_timing_rule(s.planned_time_rule)
  ) then raise exception 'Every workflow step needs a valid timing rule' using errcode='23514'; end if;
