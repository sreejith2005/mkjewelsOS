-- Roster reconciliation (0168) failed in production on the buddy-scope trigger:
-- a buddy must share the employee's branch and department, and the roster
-- moves some employees (and sometimes their buddy) to a new department. The
-- trigger checks each write against the partner's *current* placement, so the
-- outcome also depended on row order. Set roster buddy links aside, write every
-- placement, then restore each link; the existing trigger stays the authority
-- on validity, and a link it rejects is dropped and audited for an admin to
-- reassign. All other behaviour is unchanged from 0168.
...
declare
  ...
  r jsonb; v_target uuid; v_manager uuid; v_caller_sub text; v_links jsonb; v_link jsonb; v_dropped jsonb;
...
begin
  ...
  -- Set roster buddy links aside so placements can change in any order.
  select coalesce(jsonb_agg(jsonb_build_object('id', id, 'buddy_id', buddy_id, 'secondary_buddy_id', secondary_buddy_id)), '[]'::jsonb) into v_links
  from user_profiles
  where id in (select (x->>'profile_id')::uuid from jsonb_array_elements(p_roster) x)
    and (buddy_id is not null or secondary_buddy_id is not null);
  update user_profiles set buddy_id = null, secondary_buddy_id = null
  where id in (select (x->>'id')::uuid from jsonb_array_elements(v_links) x);
  ...
  for r in select value from jsonb_array_elements(p_roster) loop
    ...
    if cardinality(v_week_off) > 0 and v_week_off is distinct from v_old.week_off then
      -- The week-off trigger rematerialises availability and moves short-
      -- deadline work, which requires a signed-in administrator. Run that one
      -- write as the system Super Admin (the audited actor above), exactly as
      -- if they had changed the week off in the app, then restore the caller.
      v_caller_sub := current_setting('request.jwt.claim.sub', true);
      ...
      perform set_config('request.jwt.claim.sub', coalesce(v_caller_sub, ''), true);
    end if;
  ...
  -- Restore each buddy link against the final placements. The buddy-scope
  -- trigger decides; only its own rejections drop a link.
  for v_link in select value from jsonb_array_elements(v_links) loop
    v_dropped := '{}'::jsonb;
    if v_link->>'buddy_id' is not null then
      begin
        update user_profiles set buddy_id = (v_link->>'buddy_id')::uuid where id = (v_link->>'id')::uuid;
      exception when sqlstate '23503' or sqlstate '23514' or sqlstate '42501' then
        v_dropped := v_dropped || jsonb_build_object('buddy_id', v_link->'buddy_id', 'buddy_reason', sqlerrm);
      end;
    end if;
    if v_link->>'secondary_buddy_id' is not null then
      begin
        update user_profiles set secondary_buddy_id = (v_link->>'secondary_buddy_id')::uuid where id = (v_link->>'id')::uuid;
      exception when sqlstate '23503' or sqlstate '23514' or sqlstate '42501' then
        v_dropped := v_dropped || jsonb_build_object('secondary_buddy_id', v_link->'secondary_buddy_id', 'secondary_buddy_reason', sqlerrm);
      end;
    end if;
    if v_dropped <> '{}'::jsonb then
      insert into audit_logs(tenant_id,actor_user_id,action,module,record_id,old_value,new_value)
      values(v_actor.tenant_id,v_actor.id,'roster_buddy_link_cleared','user_management',(v_link->>'id')::uuid,v_link,v_dropped);
    end if;
  end loop;
