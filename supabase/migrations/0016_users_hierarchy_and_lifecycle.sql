-- Production Users module: reporting hierarchy, account lifecycle, transfer
-- history and role-safe audited mutations. This extends (never rewrites) the
-- Phase 1 profile contract.
set search_path = public, extensions;

do $$ begin
  create type user_account_status as enum ('active', 'invited', 'inactive', 'suspended', 'left');
exception when duplicate_object then null;
end $$;

alter table user_profiles
  add column if not exists account_status user_account_status not null default 'active',
  add column if not exists reports_to_user_id uuid references user_profiles(id);

update user_profiles
set account_status = case
  when working_status = 'resigned' then 'left'::user_account_status
  when is_login_enabled is false or working_status = 'inactive' then 'inactive'::user_account_status
  else 'active'::user_account_status
end
where account_status = 'active';

alter table user_profiles
  drop constraint if exists user_profiles_reports_to_not_self;
alter table user_profiles
  add constraint user_profiles_reports_to_not_self check (reports_to_user_id is null or reports_to_user_id <> id);

create index if not exists idx_user_profiles_reports_to on user_profiles (tenant_id, reports_to_user_id);
create index if not exists idx_user_profiles_account_status on user_profiles (tenant_id, account_status, branch_id, department_id);
...
create table if not exists user_organization_history (
  ...
  old_designation_id uuid references dropdown_masters(id),
  new_designation_id uuid references dropdown_masters(id),
  old_reports_to_user_id uuid references user_profiles(id),
  new_reports_to_user_id uuid references user_profiles(id),
  effective_from date not null default current_date,
  reason text,
  changed_by uuid not null references user_profiles(id),
  created_at timestamptz not null default now()
);
alter table user_organization_history enable row level security;
revoke all on table user_organization_history from anon, authenticated;
grant select on table user_organization_history to authenticated;
create index if not exists idx_user_organization_history_profile on user_organization_history (tenant_id, user_profile_id, effective_from desc);

create or replace function is_reporting_descendant(p_manager_id uuid, p_candidate_id uuid)
returns boolean language sql stable security definer set search_path = public as $$
  with recursive tree(id) as (
    select id from user_profiles where reports_to_user_id = p_manager_id
    union all
    select child.id from user_profiles child join tree parent on child.reports_to_user_id = parent.id
  ) select exists (select 1 from tree where id = p_candidate_id);
$$;

create or replace function current_profile_is_active()
returns boolean language sql stable security definer set search_path = public as $$
  select coalesce((
    select is_login_enabled is true
      and working_status <> 'resigned'
      and account_status = 'active'::user_account_status
    from user_profiles where auth_user_id = auth.uid() limit 1
  ), false);
$$;

-- Managers can see their full reporting tree, while HR/admin/super-admin can
-- see their tenant. Normal users still retain access to their own profile.
drop policy if exists up_select on user_profiles;
create policy up_select on user_profiles for select to authenticated using (
  (select current_profile_is_active()) and (
    auth_user_id = auth.uid()
    or (tenant_id = (select current_tenant_id()) and (select current_role_level()) in ('super_admin','admin','hr'))
    or ((select current_role_level()) = 'manager' and tenant_id = (select current_tenant_id())
      and (select is_reporting_descendant((select (current_profile()).id), id)))
  )
);

create policy user_organization_history_select on user_organization_history for select to authenticated using (
  (select current_profile_is_active()) and tenant_id = (select current_tenant_id()) and (
    user_profile_id = (select (current_profile()).id)
    or (select current_role_level()) in ('super_admin','admin','hr')
    or ((select current_role_level()) = 'manager' and (select is_reporting_descendant((select (current_profile()).id), user_profile_id)))
  )
);
...
begin
  ...
  if p_changes ? 'personal_mobile' and btrim(p_changes->>'personal_mobile') !~ '^\\+?[0-9][0-9 ()-]{7,19}$' then raise exception 'Personal mobile format is invalid' using errcode='22023'; end if;
...
revoke all on function is_reporting_descendant(uuid,uuid) from public;
revoke all on function update_user_profile_with_audit(uuid,jsonb) from public;
grant execute on function is_reporting_descendant(uuid,uuid) to authenticated;
