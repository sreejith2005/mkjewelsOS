-- Conditional form branching: make an unmatched route visible, record the route
-- an instance actually took, close the stale-option hole in the publish gate,
-- and let a multi-select answer decide a route.
--
-- Everything here is additive to 0123's stage-level routing:
--   * a stage with no rows in `fms_branch_rules` behaves exactly as before;
--   * a routed stage that matches a rule, or falls back to
--     `default_next_stage_id`, behaves exactly as before;
--   * the previously silent case — a routed stage where nothing matched and no
--     fallback exists — now stops the instance visibly instead of marking it
--     completed;
--   * a stage that routes on its own linked Form's answers now requires that
--     form to be submitted before it can be completed. Without the answers the
--     routing has nothing to read, so the old path silently took the fallback.
--     This is a deliberate behaviour change for such steps on live flows; every
--     other step, including one with an optional unrouted form, is unaffected.
-- No data is rewritten and no column is dropped or renamed.
set search_path = public, extensions;

/**
 * The option values a route expects, as a jsonb array. `in` routes persist their
 * list as JSON text in the `text` column; every other operator matches exactly
 * one value. Used by both matching and the publish-time option check.
 */
create or replace function fms_route_expected_values(p_operator text,p_value text)
returns jsonb language plpgsql stable set search_path=public as $$
declare v_parsed jsonb;
begin
  if p_value is null then return '[]'::jsonb; end if;
  if p_operator<>'in' then return jsonb_build_array(to_jsonb(p_value)); end if;
  begin
    v_parsed := p_value::jsonb;
  exception when others then
    return (select coalesce(jsonb_agg(to_jsonb(btrim(item))),'[]'::jsonb) from unnest(string_to_array(p_value,',')) item where btrim(item)<>'');
  end;
  if jsonb_typeof(v_parsed)='array' then return v_parsed; end if;
  return jsonb_build_array(to_jsonb(p_value));
end $$;

/**
 * Answer matching. Scalar answers keep 0010's behaviour exactly; a multi-select
 * answer arrives as a jsonb array, where flattening it to its JSON text made
 * `equals`/`in` unmatchable. Arrays are now compared by membership, which is
 * what `evaluateFmsBranchRule` in @jewelos/core already does.
 */
create or replace function fms_rule_matches(p_operator text,p_expected text,p_actual jsonb)
returns boolean language plpgsql stable set search_path=public as $$
declare
  v_text text := case when p_actual is null or p_actual='null'::jsonb then null else p_actual#>>'{}' end;
  v_is_array boolean := jsonb_typeof(p_actual)='array';
  v_expected_list jsonb;
begin
  if p_operator='default' then return true; end if;
  if v_is_array then
    v_expected_list := fms_route_expected_values(p_operator,p_expected);
    case p_operator
      when 'equals' then return p_actual @> jsonb_build_array(to_jsonb(p_expected));
      when 'not_equals' then return not (p_actual @> jsonb_build_array(to_jsonb(p_expected)));
      when 'contains' then return p_actual @> jsonb_build_array(to_jsonb(p_expected));
      when 'in' then return exists(select 1 from jsonb_array_elements(p_actual) choice where v_expected_list @> jsonb_build_array(choice));
      when 'not_empty' then return jsonb_array_length(p_actual)>0;
      else return false;
    end case;
  end if;
  case p_operator
    when 'equals' then return v_text is not distinct from p_expected;
    when 'not_equals' then return v_text is distinct from p_expected;
    when 'contains' then return coalesce(v_text,'') like '%'||coalesce(p_expected,'')||'%';
    when 'greater_than' then return v_text::numeric>p_expected::numeric;
    when 'greater_than_or_equal' then return v_text::numeric>=p_expected::numeric;
    when 'less_than' then return v_text::numeric<p_expected::numeric;
    when 'less_than_or_equal' then return v_text::numeric<=p_expected::numeric;
    when 'in' then return coalesce(to_jsonb(v_text)<@p_expected::jsonb,false);
    when 'not_empty' then return v_text is not null and v_text<>'';
    else return false;
  end case;
...
/**
 * The first matching route for a completed stage, as the rule itself rather than
 * only its destination, so the completion can record which branch was taken.
 * Returns no row when the stage has no routes or none of them match.
 */
create or replace function fms_stage_matched_route(p_stage_id uuid, p_instance_id uuid, p_instance_stage_id uuid, p_outcome text)
returns table(rule_id uuid, next_stage_id uuid) language plpgsql stable security definer set search_path = public as $$
declare
  ...
  v_actual jsonb;
begin
  if not exists(select 1 from fms_branch_rules where fms_stage_id = p_stage_id) then return; end if;
  ...
  for v_rule in select * from fms_branch_rules where fms_stage_id = p_stage_id order by sort_order, id loop
    ...
    if fms_rule_matches(v_rule.condition_operator, v_rule.condition_value, v_actual) then
      rule_id := v_rule.id; next_stage_id := v_rule.next_stage_id; return next;
      return;
    end if;
  end loop;
  return;
...
alter function fms_route_expected_values(text,text) owner to postgres;
alter function fms_rule_matches(text,text,jsonb) owner to postgres;
alter function fms_stage_matched_route(uuid,uuid,uuid,text) owner to postgres;
revoke all on function fms_stage_matched_route(uuid,uuid,uuid,text) from public, anon, authenticated, service_role;

-- Replaces 0123's definition. Two behavioural changes, both on routed stages:
-- the winning rule is recorded on the instance stage, and a routed stage that
-- matches nothing and has no fallback holds the instance instead of completing
-- it. Unrouted stages are untouched.
...
declare v_actor public.user_profiles; v_instance_stage public.fms_instance_stages; v_instance public.fms_instances; v_stage public.fms_stages; v_item record; v_satisfied boolean; v_next uuid; v_child uuid; v_decision_options jsonb; v_decision_outcome text; v_route record; v_routed boolean;
begin
 ...
 -- A step whose routing reads its own linked Form cannot decide anything without that Form.
 if v_stage.form_template_id is not null
    and exists(select 1 from public.fms_branch_rules where fms_stage_id=v_stage.id and source_type='form_answer')
    and not exists(select 1 from public.form_submissions where form_template_id=v_stage.form_template_id and linked_module='fms_stage' and linked_record_id=p_instance_stage_id)
 then raise exception 'This step routes on its form answers, so submit the linked form to complete it' using errcode='23514'; end if;
 ...
 v_routed=exists(select 1 from public.fms_branch_rules where fms_stage_id=v_stage.id);
 select * into v_route from public.fms_stage_matched_route(v_stage.id,v_instance.id,p_instance_stage_id,p_outcome);
 if v_route.next_stage_id is not null then
   v_next=v_route.next_stage_id;
   update public.fms_instance_stages set branch_rule_id=v_route.rule_id where id=p_instance_stage_id;
   insert into public.fms_stage_logs(fms_instance_stage_id,actor_id,action,details) values(p_instance_stage_id,v_actor.id,'route_taken',jsonb_build_object('branch_rule_id',v_route.rule_id,'next_stage_id',v_next));
 else
   v_next=v_stage.default_next_stage_id;
 end if;
 -- A routed step that matched nothing and has no fallback must never look finished.
 if v_next is null and v_routed then
   update public.fms_instances set status='on_hold',updated_at=now() where id=v_instance.id;
   insert into public.fms_stage_logs(fms_instance_stage_id,actor_id,action,details) values(p_instance_stage_id,v_actor.id,'route_unmatched',jsonb_build_object('stage_id',v_stage.id,'outcome',left(coalesce(p_outcome,''),500)));
   insert into public.notifications(tenant_id,user_profile_id,event_type,title,message,link_url,channel,delivered_status)
   values(v_instance.tenant_id,v_instance.started_by,'fms_stage_notification','Workflow needs attention',
     'No route matched after "'||coalesce(v_stage.name,'a step')||'" in '||coalesce(v_instance.reference_number,'this workflow')||'. Add an Otherwise route to the workflow, then resume it.',
     '/tasks?view=fms&instance='||v_instance.id,'in_app','delivered');
   insert into public.audit_logs(tenant_id,actor_user_id,action,module,record_id,new_value) values(v_instance.tenant_id,v_actor.id,'fms_route_unmatched','fms_instance_stages',p_instance_stage_id,jsonb_build_object('stage_id',v_stage.id,'instance_id',v_instance.id));
   return;
 end if;
 ...
 insert into public.audit_logs(tenant_id,actor_user_id,action,module,record_id,new_value) values(v_instance.tenant_id,v_actor.id,'fms_stage_completed','fms_instance_stages',p_instance_stage_id,jsonb_build_object('outcome',left(coalesce(p_outcome,''),500),'next_stage_id',v_next,'branch_rule_id',v_route.rule_id));
end $$;
...
-- Replaces 0123's definition. Adds the two checks a branched workflow needs and
-- that only the browser performed before: every routed step must have somewhere
-- for an unmatched answer to go, and a route may not match an option the linked
