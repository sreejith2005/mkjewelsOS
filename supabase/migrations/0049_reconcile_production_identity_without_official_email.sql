-- The legacy optional official_email constraint rejects otherwise valid email
-- values. Login identity is user_profiles.email, so retain the reconciliation
-- but do not modify the optional contact field.
set search_path = public, extensions;

create or replace function reconcile_duplicate_production_identity_profiles()
returns void language plpgsql security definer set search_path = public as $$
declare
  v_admin user_profiles; v_old_suganiya user_profiles; v_old_kumar user_profiles; v_suganiya user_profiles; v_kumar user_profiles;
begin
  ...
  update user_profiles set email='retired-mk0016@mkjewels.invalid', user_role='staff', account_status='inactive', is_login_enabled=false, updated_by=v_admin.id, updated_at=now() where id=v_old_suganiya.id;
  update user_profiles set email='retired-mk0027@mkjewels.invalid', account_status='inactive', is_login_enabled=false, updated_by=v_admin.id, updated_at=now() where id=v_old_kumar.id;
  update user_profiles set email='mis@mkjewels.in', updated_by=v_admin.id, updated_at=now() where id=v_admin.id;
  update user_profiles set email='suganiyamkjewels@gmail.com', updated_by=v_admin.id, updated_at=now() where id=v_suganiya.id;
  update user_profiles set email='kumarmkjewels@gmail.com', updated_by=v_admin.id, updated_at=now() where id=v_kumar.id;
