-- Roster reconciliation (0167) failed in production when a new week off fell
-- on today or tomorrow: the week-off trigger's coverage step requires a signed-
-- in administrator, and the service-role caller has none. Re-create the RPC so
-- that write runs as the system Super Admin actor. All other behaviour is
-- unchanged from 0167.
...
declare
  ...
  r jsonb; v_target uuid; v_manager uuid; v_caller_sub text;
