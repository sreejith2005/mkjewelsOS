...
export function FMSBuilderPage() {
  ...
  return <section className="mx-auto w-full max-w-7xl space-y-5">
    ...
    {startFlowId && data && profile ? <Modal onClose={() => setStartFlowId(null)} title="Start workflow"><FmsStartDialog data={data} initialFlowId={startFlowId} profile={profile} onClose={() => setStartFlowId(null)} onStarted={async (result) => { setStartFlowId(null); setStartedInstanceId(result.instance_id); setQuery(""); setView("live"); await refresh(); }} /></Modal> : null}
  </section>;
}
