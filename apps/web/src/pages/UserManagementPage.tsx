...
function EditUser({ user, data, superAdmin, onClose, onSaved }: { user: UserProfile; data: Data; superAdmin: boolean; onClose: () => void; onSaved: () => Promise<void> }) {
  const [draft, setDraft] = useState({ employee_name: user.employee_name, employee_code: user.employee_code, branch_id: user.branch_id, department_id: user.department_id, designation_id: user.designation_id ?? "", buddy_id: user.buddy_id ?? "", secondary_buddy_id: user.secondary_buddy_id ?? "", reports_to_user_id: user.reports_to_user_id ?? "", account_status: user.account_status, user_role: user.user_role, personal_mobile: user.personal_mobile ?? "", official_mobile: user.official_mobile ?? "", week_off: user.week_off.map((day) => day.toLowerCase()) });
  ...
  return <Modal onClose={onClose} title={`Edit ${user.employee_name}`} wide>
    <form className="space-y-5" onSubmit={submit}>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <Field label="Branch"><select className="field" value={draft.branch_id} onChange={(event) => { set("branch_id", event.target.value); set("department_id", ""); set("buddy_id", ""); set("secondary_buddy_id", ""); }}>{data.branches.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
        <Field label="Department"><select className="field" value={draft.department_id} onChange={(event) => { set("department_id", event.target.value); set("buddy_id", ""); set("secondary_buddy_id", ""); }}>{departments.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
        <Field label="Designation"><select className="field" value={draft.designation_id} onChange={(event) => { set("designation_id", event.target.value); set("buddy_id", ""); set("secondary_buddy_id", ""); }}><option value="">None</option>{data.dropdowns.filter((item) => item.master_type === "designation" && item.is_active).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></Field>
        ...
        <Field label="Primary buddy"><select className="field" value={draft.buddy_id} onChange={(event) => set("buddy_id", event.target.value)}><option value="">No primary buddy</option>{buddyProfiles.filter((item) => item.id !== draft.secondary_buddy_id).map((item) => <option key={item.id} value={item.id}>{item.employee_name}</option>)}</select></Field>
        <Field label="Secondary buddy"><select className="field" value={draft.secondary_buddy_id} onChange={(event) => set("secondary_buddy_id", event.target.value)}><option value="">No secondary buddy</option>{buddyProfiles.filter((item) => item.id !== draft.buddy_id).map((item) => <option key={item.id} value={item.id}>{item.employee_name}</option>)}</select><p className="mt-1 text-xs text-soft-grey">Fallback order: primary, secondary, then reporting manager.</p></Field>
