...
import { chunkTaskImportRows, isTaskImportDraftSourceFormat, kolkataDateKey, type TaskImportDraftRow, type TaskImportTimingPresetKey, type TaskImportTimingPresets } from "@jewelos/core";
...
export function TaskBulkImportPage({ onBack }: { onBack: () => void }) {
  ...
  const upload=async(file?:File)=>{if(!file)return;const presets:TaskImportTimingPresets={};setSourceFile(file);setTimingPresets(presets);setRequiredTimingPresets([]);await parseSelectedFile(file,startDate,presets,true);};
