...
export function TaskBulkImportPage({ onBack }: { onBack: () => void }) {
  ...
  const importCurrentSheet=async()=>{if(!readyRows.length)return;setBusy(true);setError(null);setResult(null);try{const started=await beginCurrentSheetTaskImport(await hashRows(readyRows),fileLabel||"task-import.csv",readyRows.length);if(started.replayed&&started.outcome!=="in_progress"&&started.outcome!=="partial"){setResult("This file was already imported. No duplicate tasks were created.");return;}const outcome=await runTaskImportChunks(started.batch_id,readyRows,(batch,rows)=>commitCurrentSheetTaskImportChunk(batch,rows as typeof readyRows),(done)=>setProgress(done));setResult(`${outcome.created} new records imported, ${outcome.replayed} duplicates skipped, and ${assigningLeft} left for later assignment.`);await refreshHistory();}catch(caught){setError(caught instanceof Error?caught.message:"Import failed");}finally{setBusy(false);}};
  ...
  return <section className="-m-4 min-h-[calc(100dvh-7.875rem)] bg-task-bg p-4 text-task-text sm:-m-6 sm:p-6"><div className="mx-auto max-w-6xl space-y-5">
    ...
    {error?<Notice tone="danger">{error}</Notice>:null}{result?<Notice tone="success">{result}</Notice>:null}
