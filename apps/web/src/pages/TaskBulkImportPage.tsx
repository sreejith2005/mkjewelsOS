...
import { TimingPresetFields } from "@/features/tasks/import/TimingPresetFields";
...
export function TaskBulkImportPage({ onBack }: { onBack: () => void }) {
  ...
  const [timingPresets,setTimingPresets]=useState<TaskImportTimingPresets>(createDefaultTaskImportTimingPresets);
  ...
  const mapped=useMemo(()=>applyIdentityMappings(eligibility.readyRows,candidates??[]),[eligibility.readyRows,candidates]);
  ...
  const importLabel=readyRows.length===draftRows.length?`Import all ${readyRows.length} record${readyRows.length===1?"":"s"}`:`Import ${readyRows.length} valid record${readyRows.length===1?"":"s"}`;
  const blockedReminder=eligibility.blockedRows.length?` ${eligibility.blockedRows.length.toLocaleString("en-IN")} source row${eligibility.blockedRows.length===1?" remains":"s remain"} blocked for correction.`:"";
  ...
  const parseSelectedFile=useCallback(async(file:File,selectedStartDate:string,presets:TaskImportTimingPresets,reset=false)=>{const requestId=++parseRequestId.current;setBusy(true);setError(null);if(reset){setResult(null);setValidation(null);setPayload(null);setDraftRows([]);setProgress(0);}try{const parsed=await parseTaskImportFile(file,{defaultStartsOn:selectedStartDate,timingPresets:presets});if(requestId!==parseRequestId.current)return;setFileLabel(file.name.replace(/[^A-Za-z0-9._ -]/g,"_"));setIssues(parsed.issues);setRequiredTimingPresets(parsed.requiredTimingPresets);if(isTaskImportDraftSourceFormat(parsed.sourceFormat)){setPayload(null);setDraftRows(parsed.draftRows);}else if(parsed.payload){setDraftRows([]);setPayload(parsed.payload);}else{setDraftRows([]);setPayload(null);setError(parsed.errors.join(" "));}}catch(caught){if(requestId===parseRequestId.current)setError(caught instanceof Error?caught.message:"Unable to read file");}finally{if(requestId===parseRequestId.current)setBusy(false);}},[]);
  const upload=async(file?:File)=>{if(!file)return;const presets=createDefaultTaskImportTimingPresets();setSourceFile(file);setTimingPresets(presets);setRequiredTimingPresets([]);await parseSelectedFile(file,startDate,presets,true);};
  const changeStartDate=async(nextDate:string)=>{setStartDate(nextDate);if(sourceFile&&nextDate)await parseSelectedFile(sourceFile,nextDate,timingPresets);};
  const changeTimingPresets=(next:TaskImportTimingPresets)=>{setTimingPresets(next);if(sourceFile)void parseSelectedFile(sourceFile,startDate,next);};
  ...
  return <section className="-m-4 min-h-[calc(100dvh-7.875rem)] bg-task-bg p-4 text-task-text sm:-m-6 sm:p-6"><div className="mx-auto max-w-6xl space-y-5">
    ...
    <Notice tone="task">Accepts the one-sheet 20-column format, six-column work list, earlier 18-column CSV, and existing canonical workbook, up to 2 MiB and 2,500 records. Blank or unclear names remain in Assigning Left.</Notice>
    ...
    <TimingPresetFields disabled={busy} onChange={changeTimingPresets} required={requiredTimingPresets} value={timingPresets}/>
    {draftRows.length&&candidates?<><div className="rounded-xl border border-task-border bg-task-bg p-4"><ImportReadinessSummary assigned={assigned} assigningLeft={assigningLeft} blocked={eligibility.blockedRows.length} ready={readyRows.length} recurring={recurring} startDate={startDate} total={draftRows.length} unresolvedLabels={mapped.unresolvedAssignees.length} unresolvedNamed={unresolvedNamed}/><div className="mt-4 flex flex-wrap gap-2"><Button disabled={!readyRows.length||busy||eligibility.globalIssues.length>0} onClick={()=>void importCurrentSheet()}><Upload className="size-4"/>{busy?`Importing ${progress}/${readyRows.length}…`:importLabel}</Button>{issues.length?<Button onClick={correction} variant="secondary"><Download className="size-4"/>Download correction report</Button>:null}</div></div><IdentityConfirmationPanel busy={busy} candidates={candidates} onConfirm={(label,userId)=>void confirmIdentity(label,userId)} unresolved={mapped.unresolvedAssignees}/></>:draftRows.length?<Notice tone="task">Matching employee names…</Notice>:null}
