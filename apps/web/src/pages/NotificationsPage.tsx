...
export function NotificationsPage({ onNavigate }: { onNavigate: (path: string) => void }) {
  ...
  const refreshAll=useCallback(async()=>{setLoading(true);setError(null);await Promise.all([refreshInbox(),refreshAdmin()]);setLoading(false);},[refreshAdmin,refreshInbox]);
