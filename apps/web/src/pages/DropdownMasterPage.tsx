...
const REQUIRED_EMPTY_CATEGORIES = ["designation"] as const;
...
export function DropdownMasterPage() {
  ...
  const categoryItems = useMemo(() => items.filter((item) => item.master_type === category), [category, items]);
  ...
  const remove = async (item: DropdownMaster) => {
    ...
    try {
      const { error: deleteError } = await supabase.rpc("change_dropdown_with_audit", {
        p_operation: "delete", p_record_id: item.id, p_master_type: item.master_type,
