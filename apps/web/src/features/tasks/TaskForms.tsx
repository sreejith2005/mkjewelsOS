...
export function DelegationTaskForm({ data, onCancel, onSave }: {
  ...
  const submit = async (event: FormEvent) => {
    ...
    setError(null);
    if (!planned || !categoryId || doers.length === 0) return setError("Choose a category, due date, and at least one doer.");
    setSaving(true);
    try {
      await onSave({ title: title.trim(), description: description.trim(), planned_datetime: new Date(planned).toISOString(), priority, category_id: categoryId, requires_upload: requiresUpload, requires_remark: requiresRemark }, doers, watchers, checklist);
    } catch (caught) {
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={(event) => void submit(event)}>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Priority"><select className="field" onChange={(event) => setPriority(event.target.value as Enums<"task_priority">)} value={priority}><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></Field>
      </div>
      <Field label="Category"><select className="field" onChange={(event) => setCategoryId(event.target.value)} required value={categoryId}><option value="">Select category</option>{data.categories.map((category) => <option key={category.id} value={category.id}>{category.label}</option>)}</select></Field>
...
export function TaskTemplateForm({ data, template, onCancel, onSave }: {
  ...
  const submit = async (event: FormEvent) => {
    event.preventDefault();
    if (!categoryId || (kind === "weekly" && weekdays.length === 0) || (assigneeType === "specific_user" && !assigneeUser) || (requiresForm && !formId)) {
    ...
    try {
      await onSave(template?.id ?? null, {
        ...
        planned_time: plannedTime, priority, category_id: categoryId, branch_id: branchId, department_id: departmentId,
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={(event) => void submit(event)}>
      ...
      <div className="grid gap-4 sm:grid-cols-3"><Field label="Priority"><select className="field" onChange={(event) => setPriority(event.target.value as Enums<"task_priority">)} value={priority}><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></Field><Field label="Branch scope"><select className="field" onChange={(event) => { setBranchId(event.target.value); setDepartmentId(""); }} value={branchId}><option value="">All branches</option>{data.branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</select></Field><Field label="Department scope"><select className="field" onChange={(event) => setDepartmentId(event.target.value)} value={departmentId}><option value="">All departments</option>{departments.map((department) => <option key={department.id} value={department.id}>{department.name}</option>)}</select></Field></div>
