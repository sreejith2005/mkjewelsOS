...
type AssigningLeftRpcClient = {
  ...
  rpc(name: "assign_imported_task_with_audit", args: { p_record_kind: string; p_record_id: string; p_user_profile_id: string }): Promise<{ data: unknown; error: { message: string } | null }>;
};

export async function loadAssigningLeftTasks(): Promise<AssigningLeftRecord[]> {
  const client = supabase as unknown as AssigningLeftRpcClient;
  return unwrap<AssigningLeftRecord[]>(await client.rpc("list_assigning_left_tasks"), "Load Assigning Left");
}

export async function assignImportedTask(recordKind: "task" | "template", recordId: string, userProfileId: string) {
  const client = supabase as unknown as AssigningLeftRpcClient;
  return unwrap<{ assignment_status: "assigned" | "assigning_left" }>(await client.rpc("assign_imported_task_with_audit", {
