...
describe("FMS web definition adapter", () => {
  ...
  it("rehydrates graph relationships while leaving legacy role assignments out of the named-user editor", () => {
    ...
    const definition=flowToDefinition(flow,data); expect(definition.version).toBe(3); expect(definition.stages[0]?.defaultNextStageKey).toBe("done"); expect(definition.stages[0]?.assigneeRules).toEqual([]); expect(definition.stages[1]?.type).toBe("end");
  });
});
