...
export function FmsStageEditor({ stage, stages, data, flowBranchId, onChange, onDelete }: { stage: FmsStageDefinition; stages: readonly FmsStageDefinition[]; data: FmsData; flowBranchId?: string | undefined; onChange: (value: FmsStageDefinition) => void; onDelete: () => void }) {
  ...
  return <div className="space-y-5 pb-8">
    ...
    {!firstStage && human ? <section className="space-y-3 border-t border-gold/15 pt-4">
      <label className={`flex items-start gap-2 rounded-xl border border-gold/20 p-3 text-sm ${!earlierDecisions.length ? "opacity-60" : ""}`}><input aria-label="Only run on a Yes or No path" checked={showConditional && !!stage.sla.conditional} disabled={!earlierDecisions.length} onChange={(event) => { setShowConditional(event.target.checked); updateSla({ conditional: event.target.checked ? { decisionStageKey: earlierDecisions.at(-1)!.key, outcome: "yes" } : undefined }); }} type="checkbox" /><span><b className="block text-white">Conditional (optional)</b><span className="text-xs text-soft-grey">Only run after a specific Yes or No decision.</span></span></label>
      {!earlierDecisions.length ? <p className="text-xs text-soft-grey">Add a Decision step (Yes/No) earlier in the workflow to enable this option.</p> : null}
      {showConditional && stage.sla.conditional && earlierDecisions.length ? <div className="grid gap-3 rounded-xl border border-gold/15 bg-white/[0.02] p-3 sm:grid-cols-2"><Field label="Earlier decision"><select className="field" onChange={(event) => updateSla({ conditional: { ...stage.sla.conditional!, decisionStageKey: event.target.value } })} value={stage.sla.conditional.decisionStageKey}>{earlierDecisions.map((item) => <option key={item.key} value={item.key}>{item.name}</option>)}</select></Field><Field label="Run when answer is"><select className="field" onChange={(event) => updateSla({ conditional: { ...stage.sla.conditional!, outcome: event.target.value as "yes" | "no" } })} value={stage.sla.conditional.outcome}><option value="yes">Yes</option><option value="no">No</option></select></Field></div> : null}
    </section> : null}
