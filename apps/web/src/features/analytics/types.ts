...
export type HomeSummary={generated_at:string;tenant_local_date:string;timezone:string;profile:{id:string;name:string;role:string;branch_id:string;branch_name:string|null;department_id:string;department_name:string|null;working_status:string};tasks:HomeTask[];fms_stages:HomeFms[];fms_starters:HomeFmsStarter[];forms_awaiting_submission:HomeForm[];crm_followups:HomeFollowup[];unread_notifications:number;availability_status:string|null;recent_activity:HomeActivity[];quick_actions:string[]};
...
export type ProgressCounts={assigned:number;completed:number;remaining:number;overdue:number;on_time_completed?:number};
export type EmployeeProgressRow=ProgressCounts&{user_profile_id:string;employee_name:string;branch_id:string;branch_name:string;department_id:string|null;department_name:string|null};
export type DepartmentProgressRow=ProgressCounts&{department_id:string;department_name:string};
export type BranchProgressRow=ProgressCounts&{branch_id:string;branch_name:string};
export type EmployeeProgress={employees:EmployeeProgressRow[];departments:DepartmentProgressRow[];branches:BranchProgressRow[]};
