      ...
      <section>
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          <ActionGroup title="My Tasks">
            {openTasks.length ? openTasks.slice(0, 4).map((t) => <ActionItem description={`Due ${when(t.due_at)}`} key={t.id} label={t.overdue ? "Overdue — open now" : "Assigned task"} onOpen={() => onNavigate("/tasks")} overdue={t.overdue} title={t.title} />) : <EmptyMessage>No tasks waiting.</EmptyMessage>}
          </ActionGroup>
