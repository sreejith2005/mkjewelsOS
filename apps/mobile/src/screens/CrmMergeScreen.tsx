...
import { crmMergeConfirmed } from "@jewelos/core";
...
export function CrmMergeScreen() {
  ...
  const search = async () => { setBusy(true); setError(null); try { setResults((await searchClients({ search: query.trim(), limit: 25 })).filter((item) => item.id !== params.survivorId)); } catch (caught) { setError(caught instanceof Error ? caught.message : "Client search failed."); } finally { setBusy(false); } };
